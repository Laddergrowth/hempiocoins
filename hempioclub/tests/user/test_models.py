...
class TestUser(object):
    ...
    def test_subscribed_user_receives_more_hccoins(self, users):
        """ Subscribed user receives more hccoins. """
        user = User.find_by_identity('admin@local.host')
        user.add_hccoins(Subscription.get_plan_by_id('bronze'))

        assert user.hccoins == 210
