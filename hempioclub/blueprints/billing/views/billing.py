...
from hempioclub.blueprints.billing.forms import SubscriptionForm, \
    UpdateSubscriptionForm, CancelSubscriptionForm, PaymentForm
from hempioclub.blueprints.billing.models.coupon import Coupon
from hempioclub.blueprints.billing.models.subscription import Subscription
from hempioclub.blueprints.billing.models.invoice import Invoice
from hempioclub.blueprints.billing.decorators import subscription_required, \
...
@billing.route('/purchase_hccoins', methods=['GET', 'POST'])
@login_required
def purchase_hccoins():
    stripe_key = current_app.config.get('STRIPE_PUBLISHABLE_KEY')
    ...
    if form.validate_on_submit():
        ...
        bundle = next((item for item in coin_bundles if
                       item['hccoins'] == coin_bundles_form), None)
        ...
        if bundle is not None:
            ...
            created = invoice.create(user=current_user,
                                     ...
                                     hccoins=coin_bundles_form,
                                     coupon=request.form.get('coupon_code'),
                                     token=request.form.get('stripe_token'))

            if created:
                flash(_('%(amount)s hccoins have been added to your account.',
    ...
    return render_template('billing/purchase_hccoins.html', form=form)
